...
- **Lockfile Conflict**: Presence of both `npm-lock.yaml` and `package-lock.json` caused ambiguity for Vercel's build detector, potentially leading to incorrect dependency resolution or installation failures.
...
- **Dependency Hoisting**: Standard npm strictness sometimes breaks packages that expect a flat `node_modules` (common in React/Radix UI ecosystems).
...
# 3. Configure npm for compatibility
cat <<EOT > .npmrc
...
1. **Local Build**: Run `npm run build`. It should complete without errors.
2. **Local Tests**: Run `npm run test`. Ensure all tests pass.
3. **Environment Check**: Run `node -v` and `npm -v` to ensure they align with `.node-version` and `packageManager`.
4. **Prisma Check**: Verify `node_modules/.prisma/client` exists after `npm install`.

## 4. Preventive Recommendations
- **Single Lockfile**: Never commit `package-lock.json` or `yarn.lock` in a npm project. Use `.gitignore` to enforce this.
...
- **CI/CD Caching**: Ensure Vercel or GitHub Actions are configured to cache `.npm-store` and `.next/cache` for faster, more reliable builds.
