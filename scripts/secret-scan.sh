...
if command -v rg >/dev/null 2>&1; then
  ...
  if grep -rn --exclude-dir=.git --exclude-dir=.next --exclude-dir=node_modules --exclude=pnpm-lock.yaml --exclude=package-lock.json --exclude=yarn.lock -E "$PATTERN" . 2>/dev/null; then
    echo "❌ Potential secret/token pattern found in tracked content."
    exit 1
  fi
fi
