...
if command -v rg >/dev/null 2>&1; then
  if rg -n --hidden --glob '!.git' --glob '!.next' --glob '!node_modules' --glob '!package-lock.json' --glob '!yarn.lock' --glob '!.github/docs' --glob '!.env.production' --pcre2 "$PATTERN" .; then
    echo "❌ Potential secret/token pattern found in tracked content."
    exit 1
  fi
else
  if grep -rn --exclude-dir=.git --exclude-dir=.next --exclude-dir=node_modules --exclude-dir=.github/docs --exclude=package-lock.json --exclude=yarn.lock --exclude=.env.production -E "$PATTERN" . 2>/dev/null; then
    echo "❌ Potential secret/token pattern found in tracked content."
    exit 1
  fi
fi
